...
varlist : varlist VIRGULA VARNAME                       {char *params=malloc(strlen($1) + strlen($3) + 7); sprintf(params, "%s, int %s", $1, $2); $$ = params; qtdParametros++;}
...
elem    : VARNAME                                       {char *variavel = malloc(strlen($1)); sprintf(variavel,"%s",$1); $$ = variavel;}
        | NUM                                           {char *numero = malloc(strlen($1)); sprintf(numero,"%d",$1); $$ = numero;}
        ;
